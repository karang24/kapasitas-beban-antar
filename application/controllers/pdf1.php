<?php 
class Pdf1 extends CI_Controller {
  ...
  function index($download_pdf=''){
    ...
    //
    $query= $this->db->query("SELECT * FROM tb_kba where nippos");
    if($query->num_rows()>0)
    ...
    $data['result']=$this->m_superv->bacadata2();
	

    $laporan_transaksi = $this->load->view('laporan/rekap_kba/laporankba1',$data,true);
